...
# Simple Face Detection using Intel(R) OpenVINO(TM) Toolkit (Windows)
In this tutorial you will build a simple application capable of detecting faces in a live camera stream using the Intel(R) OpenVINO(TM) Toolkit and an Intel(R) pretrained model. Once completed you will have an understanding of the code required to start creating your own OpenVINO(TM) based solutions and how to target various accelerators in the Intel(R) portfolio (CPU, GPU, VPU).
...
```
 ...
 - The next step is to read the model we are going to use for face detection. This is an Intel(R) provided pretrained model (SSD model with a MobileNet backbone) which is already in the required Intermediate Representation (IR) format. The Intermediate Representation consists of an XML file which describes the network layout and layers and a binary file which contains the model weights.
